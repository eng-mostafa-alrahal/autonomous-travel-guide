...
| FR-P2 | Required slots are **destination** (city and/or country), **origin** (`origin_city` — traveller's current / departure city), **`num_days`**, and **`budget`** (free text, e.g. `"$1500"` or `"mid-range"`). Optional slots: `start_date`, `party_size`, `interests`. Origin aliases (`origin`, `departure_city`, `from_city`, `home_city`) MUST coerce into `origin_city`. |
| FR-P3 | While slots are still needed, the planner MUST pause with a LangGraph `interrupt()` (`ask_requirements`), ask the user **one friendly question at a time** (not a multi-slot checklist), and loop back to extraction on resume. After hard slots are complete, it SHOULD soft-ask once for `interests` (`preferences_asked`); an empty answer after that MUST NOT block planning. |
| FR-P3b | After requirements are complete and before specialists run, `confirm_destination` MUST resolve the destination (places provider when available, else LLM). It MUST ask via `interrupt()` (`kind: destination_confirm`) when the place is missing, looks misspelled, or is ambiguous (same city name in multiple countries, or a name that is both a city and a country). On confirm it MUST write canonical `destination_city` / `destination_country` and set `destination_confirmed`. |
| FR-P4 | Once the destination is confirmed, `city_expert` MUST run **first** (it gates on a KB miss), after which `hotels`, `flights_logistics`, and `food` MUST fan out to run **concurrently** via LangGraph `Send`. Dispatch is deterministic — every section is produced exactly once, with no per-step LLM routing. |
...
| FR-P6 | On a KB miss, `city_expert` MUST set `kb_miss = true` and a canonical `destination_key` in state so the master graph can trigger the Knowledge Builder; the planner MUST then end before the parallel fan-out fires. User-facing copy for this hand-off MUST avoid backend jargon. |
| FR-P7 | `hotels`, `flights_logistics`, and `food` are search-then-summarize specialists sharing one parameterized prompt (`travel_specialist`, parameter `role`) and one node factory parameterized by `role`. `flights_logistics` MUST use the collected `origin_city` for inbound routes (plus local transport once there) and MUST use `LOGISTICIAN_MODEL` when set. |
| FR-P8 | `synthesize_itinerary` MUST compose a **full draft** day-by-day plan from all `specialist_outputs`, respecting budget and `num_days`, write it to `itinerary` and as the final `AIMessage`, and **invite the user to refine**. The draft MUST include Getting there, Getting around, Where to stay, and a Rough cost sketch (with $ figures when specialists provided prices). If the model omits travel/costs, a deterministic appendix MUST be appended. It MUST group geographically-nearby POIs into the same day using their coordinates. |
...
| FR-K4 | `deduplicate` MUST store each DeepSearch report verbatim and MUST call structured LLM output (`PreparedKnowledge`) **per research theme** to add extra long-form chapters from the model's own knowledge (never summarize the research away). It uses `VALIDATOR_MODEL` when set. |
| FR-K5 | `ingest` MUST send segments through the `IIngestionService` port using the already-prepared research segments. On transient failure it MUST retry the **same** segments up to **2 more times** (3 attempts total) without re-running deep research or deduplicate. On success it marks `kb_destinations.status = ready` with `doc_count`; only after all attempts fail it marks `status = failed`. |
...
| FR-A3 | When a graph interrupts (requirements question, destination confirm, or KB-build confirmation), the API MUST return `interrupted: true` plus the interrupt payload; the client resumes via `POST /runs/{thread_id}/resume` with `{"action": "approve"|"reject", "feedback": "..."}` (or destination pick fields). `GET /runs/{thread_id}/state` exposes the paused state. Interrupt payloads MUST carry a `kind` (`requirements` \| `destination_confirm` \| `kb_build`) so clients can render the right prompt. |
...
| FR-A7 | `GET /sessions/{session_id}/messages` MUST return the session's conversation transcript from the checkpointer (owner-only). Internal memory-summary messages MUST be omitted; sessions with no checkpoint yet MUST return an empty list. |

### 3.5 Progress streaming ✅

| ID | Requirement |
|----|-------------|
| FR-S1 | Graph nodes MUST attach a coarse `phase` (`requirements`, `planning`, `knowledge_build`, `itinerary`, `done`) and a **plain-language** user-facing `phase_status` line to their state updates (no backend jargon such as “specialist”, “knowledge base”, or node names). Phase constants and status builders live in the **domain** layer (`domain/phases.py`, no framework imports). |
...
| FR-I5 | **Embedding consistency (critical):** the adapter MUST pin `embedder_provider`, `embedding_model`, `embedding_dimensions`, and `embedding_pipeline` on every submit to match query-time settings. When `INGESTION_EMBEDDING_PIPELINE=late_chunking` it MUST also send `late_chunk_min_tokens` / `late_chunk_max_tokens` and use Jina at query time. It MUST raise if any returned vector width differs from `EMBEDDING_DIMENSIONS`. |
...
| 10 — Real APIs | **Partial ✅** — provider seam + per-provider flags + **`TRAVEL_MOCK_APIS`** offline pack (hotels/flights/places/transit for London, Paris, Rome, Berlin, New York, Damascus, Los Angeles). Live free adapters: **places** (Nominatim) and **transit** (OSRM). **Flights/hotels live adapters 🔜** (need keys). KB builds are unchanged by mocks. |
